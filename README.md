...
英語での診療スキル（問診・身体診察・症例プレゼンテーション）を総合的に学習するためのインタラクティブなWebアプリです。各英語表現をクリックすると高品質な音声が再生され、発音とイントネーションを学習できます。
...
│   └── OPQRSTフレームワーク、系統的な質問集
├── 身体診察（Physical Examination）
│   ├── 頭頸部（Head & Neck）
│   ├── 胸部（Chest）
│   ├── 腹部（Abdomen）
│   └── 神経（Neurological）
└── プレゼン（Case Presentation）
    ├── 全体像（SAD原則、12項目、Diagnostic Framing）
    ├── テンプレート（定型表現集）
    ├── SLE症例（7セクション×音声）
    ├── RA症例（7セクション×音声）
    ├── 場面別（SNAPPS / I-PASS / SBAR）
    └── ボキャブラリー（267語）
```

## 機能

- **問診**: OPQRSTフレームワーク、系統的な質問集（便・尿・体重・食欲・睡眠、家族歴、既往歴、薬、健康診断、対処行動、酒タバコ、仕事、ストレス・睡眠、生理・性生活など）
- **身体診察**:
