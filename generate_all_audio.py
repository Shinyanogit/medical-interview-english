...
def extract_questions_from_html(html_file):
    ...
    for item in question_items:
        ...
        if data_text:
            ...
            # ()内のパラフレーズを削除（ただし、文の構造を壊さないように注意）
            # まず、引用符で囲まれた部分を保護
...
def main():
    ...
    # 既存のマッピングにない質問のみを抽出
    new_questions = [q for q in unique_questions if q not in audio_map]
    print(f"新規質問数: {len(new_questions)}件")
    print(f"既存質問数: {len(unique_questions) - len(new_questions)}件\n")

    if new_questions:
        # 音声ファイルを生成
        audio_map = generate_audio_files(new_questions, audio_map, overwrite=False)
    else:
        print("新規の質問はありません。すべての音声ファイルは既に生成されています。")
