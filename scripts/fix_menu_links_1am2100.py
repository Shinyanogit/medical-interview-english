#!/usr/bin/env python3
"""
1AM2100のメニューページのリンクをローカルファイルパスに変換するスクリプト
"""
import os
import re
...
BASE_DIR = Path("/Users/shinyyama/Documents/プログラミング/medical-interview-english/local/restored/1AM2100/S2")

def find_id_in_files(id_pattern: str) -> Optional[tuple]:
    """
    IDを含むHTMLファイルを検索し、ファイルパスとアンカーを返す
    """
    # IDパターンからセクションを特定（例: S2A00100M00000 -> A）
    # S2Zで始まるIDは0セクション（introduction）を指す可能性がある
    if id_pattern.startswith('S2Z') and len(id_pattern) > 3:
        # S2Z10100M00000 -> 0セクションを試す
        section = '0'
    else:
        match = re.match(r'S2([A-Z0-9])(\d+)M\d+', id_pattern)
        if not match:
            return None
