#!/usr/bin/env python3
"""
1A662F0のメニューページのリンクをローカルファイルパスに変換するスクリプト
"""
import os
import re
...
def find_id_in_files(id_pattern: str) -> Optional[tuple]:
    ...
    """
    # IDパターンからセクションを特定（例: B6A00100M00000 -> 01）
    # B6のIDパターンを解析
    match = re.match(r'B6([A-Z])(\d+)M\d+', id_pattern)
    if not match:
        return None
    
    section_letter = match.group(1)
    # セクション文字をディレクトリ番号にマッピング
    section_map = {
    ...
    section_dir_name = section_map.get(section_letter)
        ...
        try:
            with open(html_file, 'r', encoding='utf-8') as f:
                ...
                if id_pattern in content:
                    ...
                    relative_path = f"../{section_dir_name}/{html_file.name}#{id_pattern}"
...
def fix_menu_file(menu_file: Path):
    ...
    # 外部URLのパターンを検索（nmrefとbmrefの両方に対応）
    pattern = r'href="http://medilink-study\.com/medilink_app/(?:nmref|bmref)\?id=([^"]+)"'
